...
from omagent_core.clients.devices.cli.client import DefaultClient  # For CLI client interface
...
workflow = ConductorWorkflow(name='fitness_helper')
...
task2 = simple_task(task_def_name='FitDecider', task_reference_name='fit_decider', inputs={'user_instruction': task1.output('user_instruction')})
# 3. Weather information retrieval
task3 = simple_task(task_def_name='FitSearcher', task_reference_name='fit_searcher', inputs={'user_instruction': task1.output('user_instruction')})
# 4. Final outfit recommendation generation
task4 = simple_task(task_def_name='FitAdvices', task_reference_name='fit_advices')
