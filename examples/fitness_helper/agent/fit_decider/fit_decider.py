...
class FitDecider(BaseLLMBackend, BaseWorker):
    ...
    def _run(self, user_instruction:str, *args, **kwargs):
        ...
        if content.get("fitInfo_provided") is not None:
            fitInfo_provided = content['fitInfo_provided']
            if fitInfo_provided:
                self.callback.info(agent_id=self.workflow_instance_id, progress='Fit Decider', message='Fitness information provided')
                return {'switch_case_value': 1}
            else:
                self.callback.info(agent_id=self.workflow_instance_id, progress='Fit Decider', message='Need web search for fitness information')
