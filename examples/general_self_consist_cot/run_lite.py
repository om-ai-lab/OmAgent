...
class SimpleCOTInputInterface(BaseWorker):

    def _run(self, query, *args, **kwargs):
        self.stm(self.workflow_instance_id)['user_question'] = query

        return {'user_question': query}
...
client_input_task = simple_task(task_def_name=SimpleCOTInputInterface, task_reference_name='input_interface')
...
cli_client = DefaultClient(interactor=workflow, config_path=config_path, workers=[SimpleCOTInputInterface()])
