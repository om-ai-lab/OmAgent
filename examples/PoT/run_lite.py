...
class PoTSimpleInterface(BaseWorker):
    def _run(self, query, *args, **kwargs):
        examples = input(f'{Fore.GREEN}Please input examples if you have, input "None" if you do not have:')        
        if examples == 'None':
            examples = None
        options = input(f'{Fore.GREEN}Please input options if you are doing a multiple choice question, input "None" if you do not have:')                
        if options == 'None':
            options = None
...
workflow = ConductorWorkflow(name='PoT', lite_version=True)

# Create input interface task to handle user interactions
client_input_task = simple_task(task_def_name=PoTSimpleInterface, task_reference_name='PoT_input_interface')
...
cli_client = DefaultClient(interactor=workflow, config_path=config_path, workers=[PoTSimpleInterface()])
