...
class DefaultCallback(CallbackBase):
    ...
    def info(self, progress, message):
        logging.info(
            f"\n{Fore.BLUE}info:{progress} {message}{Style.RESET_ALL}"
        )
    ...
    def send_block(
        ...
        **kwargs
        ...
        logging.info(f"\n{Fore.BLUE}block:{json.dumps(msg, indent=2, ensure_ascii=False)}{Style.RESET_ALL}")
        

    def error(self, error_code, error_info, **kwargs):
        logging.error(f"\n{Fore.RED}{error_info}{Style.RESET_ALL}")

    def send_answer(self, msg, **kwargs):
