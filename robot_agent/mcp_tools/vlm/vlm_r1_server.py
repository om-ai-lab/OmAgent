...
def is_base64_string(s: str) -> bool:
    ...
    try:    
        return bool(re.match(r'^data:image/(jpeg|png|gif|webp|bmp);base64,', s))
    except:
        return False
...
async def analyze_image(
    ...
    elif len(image_path) > 100:
        try:
            local_image_path = save_base64_image(image_path)
        except Exception as e:
            return {"error": f"Failed to save image from base64 string: {str(e)}"}
...
def main():
    ...
    try:
        mcp.run(transport="sse", host="0.0.0.0", port=8009)
    finally:
